...
def analyze_with_groq(groq_key: str, image_bytes: bytes) -> NutritionData:
    ...
    # Актуальный список Vision-моделей Groq (с автоперебором)
    candidate_models = [
        "qwen/qwen3.6-27b",
    ...
    for model_name in candidate_models:
        ...
        try:
            ...
            # Переходим к следующей модели при устаревшей или отключенной модели
            if e.code in (400, 404) or "model_decommissioned" in err_body or "model_not_found" in err_body:
                last_error_msg = f"[{model_name}]: {err_body}"
                continue
    ...
    raise HTTPException(status_code=400, detail=f"Ошибка Groq API (все Vision модели недоступны): {last_error_msg}")
